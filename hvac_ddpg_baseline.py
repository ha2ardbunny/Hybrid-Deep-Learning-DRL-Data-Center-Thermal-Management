...
# ─── Load CNN-LSTM Model & Scalers (for DUE evaluation only) ─────────────────
# NOTE: CNN-LSTM predictions are NOT used in the DDPG state vector.
#       They are pre-computed here solely for DUE metric calculation at test time,
#       to allow fair comparison against the CNN-LSTM + DDPG integrated model (V1).

print("Loading CNN-LSTM model (for DUE evaluation only)...")
...
# ─── Pre-compute CNN-LSTM Predictions (for DUE evaluation only) ──────────────

print("\n" + "="*60)
print("Baseline Model: Pure DDPG (no CNN-LSTM in state vector)")
print("  State: 7 features (CNN-LSTM predicted_t EXCLUDED)")
print("  CNN-LSTM predictions pre-computed for DUE metric ONLY")
print("="*60)

print("\nPre-computing CNN-LSTM predictions (DUE evaluation only)...")
...
print(f"Pre-computed {len(all_predictions)} predictions (for DUE only, not used in state)")
print(f"Prediction range: {all_predictions.min():.2f} - {all_predictions.max():.2f}degC")

# ─── HVAC Environment (Pure DDPG — no CNN-LSTM in state) ─────────────────────
...
class HVACEnv:
    ...
    def __init__(self, df, feature_cols,
        ...
        # KEY DIFFERENCE: state_dim = 7 (no predicted_t from CNN-LSTM)
        self.state_dim  = 7
        self.action_dim = 1
    ...
    def _build_state(self, idx):
        """
        Pure DDPG state — 7 features, no CNN-LSTM prediction.

        Compared to V1 (CNN-LSTM + DDPG), predicted_t is removed.
        The agent must rely entirely on current observed measurements
        without any predictive look-ahead from the CNN-LSTM.
        """
    ...
    def reset(self):
        ...
        return self._build_state(self.current_idx)
    ...
    def step(self, action):
        ...
        # 5. Reward = comfort + energy (identical to V1 for fair comparison)
        ...
        # 6. Next state (7-dim, no CNN-LSTM)
...
def train(df_model, total_steps=200000):
    ...
    print(f"\n{'='*60}")
    print(f"Pure DDPG Baseline Training — {total_steps} steps")
    print(f"  State dim:      {env.state_dim} features (no CNN-LSTM predicted_t)")
    ...
    print(f"  Reward:         -pue_penalty - comfort_penalty - thermal_penalty")
    ...
    pbar = tqdm(total=total_steps, desc='Training (Baseline)', unit='step')
    ...
    import json
    train_results = {
        'model':              'Pure DDPG Baseline (no CNN-LSTM)',
    ...
    with open('ddpg_baseline_train_results.json', 'w') as f:
        json.dump(train_results, f, indent=2)

    np.save('ddpg_baseline_train_rewards.npy', np.array(episode_rewards))
    np.save('ddpg_baseline_train_errors.npy',  np.array(error_history))
    np.save('ddpg_baseline_train_temps.npy',   np.array(temp_history))
    np.save('ddpg_baseline_train_supply.npy',  np.array(sp_history))
    np.save('ddpg_baseline_train_pues.npy',    np.array(pue_history))
    print("Baseline training results saved:")
    print("  ddpg_baseline_train_results.json")
    print("  ddpg_baseline_train_rewards.npy")
    print("  ddpg_baseline_train_errors.npy")
    print("  ddpg_baseline_train_temps.npy")
    print("  ddpg_baseline_train_supply.npy")
    print("  ddpg_baseline_train_pues.npy")
...
def _plot_training(episode_rewards, episode_pues,
    ...
    fig.suptitle('DDPG Training Results — Pure DDPG Baseline (No CNN-LSTM)\n',
    ...
    plt.show()
    print("Plot saved -> baseline_training_results.png")
...
def test(df_model, num_episodes=5):
    ...
    for ep in range(num_episodes):
        ...
        # DUE: compare simulated T_Return vs CNN-LSTM predicted T_Return
    ...
    plt.tight_layout()
    plt.savefig('baseline_test_results.png', dpi=150, bbox_inches='tight')
    plt.show()
    print("Plot saved -> baseline_test_results.png")

    print(f"\n{'='*55}")
    print(f"Baseline Test Summary ({num_episodes} episodes)")
    print(f"{'='*55}")
    ...
    with open('ddpg_baseline_test_results.json', 'w') as f:
        json.dump(test_results, f, indent=2)

    np.save('ddpg_baseline_test_errors.npy',   np.array(all_errors))
    np.save('ddpg_baseline_test_pues.npy',     np.array(all_pues))
    np.save('ddpg_baseline_test_rewards.npy',  np.array(all_rewards))
    print("\nBaseline test results saved:")
    print("  ddpg_baseline_test_results.json")
    print("  ddpg_baseline_test_errors.npy")
    print("  ddpg_baseline_test_pues.npy")
    print("  ddpg_baseline_test_rewards.npy")

# ─── Main ────────────────────────────────────────────────────────────────────

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='HVAC Pure DDPG Baseline')
